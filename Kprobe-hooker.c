...
static int entry_handler(struct kretprobe_instance *ri,struct pt_regs *regs){
    printk(KERN_INFO "enter exec (pid: %d, parent pid: %d)\n", current->pid, current->parent->pid);
    
    const char *argv_ptr;
    const char ** argv_ptr2;
    // char *argv_res = NULL;
    char *native = NULL;
    ssize_t argv_len = 0,argv_res_len = 0,i = 0, len = 0,offset = 0;

    const char __user **argv = (const char __user **)regs->di;
    char __user *arg;
    ...
    // 获取第一个参数
    if (get_user(arg, argv) == 0) {
        if (strncpy_from_user(argv_ptr, arg, 255) > 0) {
            pr_info("argv_ptr: %s\n", argv_ptr);
        }
    }
    /*
        // 获取 execve 的第二个参数 (argv)
     const char ** argv = (const char **)regs->si;

    // 逐个打印 argv 数组中的命令行参数
    if (argv != NULL) {
        pr_info("Command line arguments:\n");
        while (argv[i] != NULL && i < MAX_ARG_STRINGS) {
            // 获取每个参数
            pr_info("argv[%d]: %s\n", i, argv[i]);
            i++;
        }
    ...
    */
    ...
    //pr_info("argv_ptr2[0] : %s", argv_ptr2[0]);
    argv_len = count(argv_ptr2, MAX_ARG_STRINGS);
    pr_info("argv_res_len:%d\n",argv_len);
...
static int ret_handler(struct kretprobe_instance *ri,struct pt_regs *regs){
    ...
    pr_info("cwd:%s\n",cwd);
    if (!argv_res) {
    	pr_info("argv_res is NULL\n");
    }
    if (!comm) {
        pr_info("comm is NULL\n");
    }
    if (!abs_path) {
        pr_info("abs_path is NULL\n");
    }
    if (!pid_tree) {
        pr_info("pid_tree is NULL\n");
    }
    if (!cwd) {
        pr_info("cwd is NULL\n");
    }
    if (!tty_name) {
        pr_info("tty_name is NULL\n");
    }
    if (!pcomm) {
        pr_info("pcomm is NULL\n");
    }
    if (!pexe) {
        pr_info("pexe is NULL\n");
    }
    if (!stdin) {
       pr_info("stdin is NULL\n");
    }
    if (!stdout) {
        pr_info("stdout is NULL\n");
    }
    


    pr_info("Total length: %zu\n", result_str_len);

    if (argv_res && comm && abs_path && pid_tree && cwd && tty_name && pcomm && pexe && stdin && stdout) {
    	result_str_len = strlen(argv_res) + strlen(comm) + strlen(abs_path) + strlen(pid_tree) + strlen(cwd) + strlen(tty_name) + strlen(pcomm)+ strlen(pexe) + strlen(stdin) + strlen(stdout) + 256;
    	pr_info("test_len:%zu\n", result_str_len);
    } else {
    	pr_info("One of the input strings is NULL\n");
    	result_str_len += strlen(argv_res ? argv_res : "");
	result_str_len += strlen(comm ? comm : "");
	result_str_len += strlen(abs_path ? abs_path : "");
	result_str_len += strlen(pid_tree ? pid_tree : "");
	result_str_len += strlen(cwd ? cwd : "");
	result_str_len += strlen(tty_name ? tty_name : "");
	result_str_len += strlen(pcomm ? pcomm : "");
	result_str_len += strlen(pexe ? pexe : "");
	result_str_len += strlen(stdin ? stdin : "");
	result_str_len += strlen(stdout ? stdout : "");

        result_str_len += 256;  // 常量偏移量
    }
